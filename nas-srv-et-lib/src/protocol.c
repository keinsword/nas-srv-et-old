...
int readMsgHeader(connection *connListItem, char *buffer) {
	int i = 0, divider = 0;

	while(buffer[i] != '\0') {
		connListItem->storageBuffer[connListItem->lengthRecv+i] = buffer[i];
		if(buffer[i] == '|')
			divider++;
		if((divider < 3) && (strlen(connListItem->storageBuffer) == MAX_HEADER_SIZE))
			return INVALID_SIGNATURE;
		if(divider == 3) {
			memset(buffer, '\0', strlen(connListItem->storageBuffer));
			return divider;
		}
		i++;
	}
	connListItem->lengthRecv = strlen(connListItem->storageBuffer);
	return 0;
}

int checkProtoSignatures(connection *connListItem) {
	int i, j = 0, k;
	char temp[BUFFERSIZE];

	for(i = 0; i < 3; i++, k = 0) {
		memset(&temp, 0, sizeof(temp));
		while(connListItem->storageBuffer[j] != '|') {
			temp[k] = connListItem->storageBuffer[j];
			j++; k++;
		}
		j++;
		switch(i) {
		case 0:
			if(strncmp(temp, PROTO_NAME, strlen(PROTO_NAME)))
				return INVALID_SIGNATURE;
			break;
		case 1:
			if(strncmp(temp, PROTO_VER, strlen(PROTO_VER)))
				return INVALID_SIGNATURE;
			break;
		case 2:
			connListItem->length = atoi(temp);
...
void readMsgPayloadAndTailer(connection *connListItem, char *buffer) {
	int i = 0;

	while(buffer[i] == '\0')
		i++;
	while((buffer[i] != '\0') && (connListItem->length != 0)) {
		connListItem->storageBuffer[connListItem->lengthRecv+i] = buffer[i];
		(connListItem->length)--;
		i++;
	}
	if(connListItem->length == 0)
		connListItem->segmentationFlag = 0;
	else
		connListItem->segmentationFlag = 1;
}

int readMsgByParts(connection *connListItem, char *buffer) {
	int result;

	if(connListItem->segmentationFlag == 0) {
		result = readMsgHeader(connListItem, buffer);
		if(result < 0)
			return result;
		else {
			result = checkProtoSignatures(connListItem);
			if(result < 0)
				return result;
			else {
				readMsgPayloadAndTailer(connListItem, buffer);
